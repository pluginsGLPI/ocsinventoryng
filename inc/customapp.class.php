 ...
 */
class PluginOcsinventoryngCustomapp extends CommonDBChild {
    ...
    * @param $force
    */
   static function updateCustomapp($computers_id, $ocsComputer, $cfg_ocs, $force) {
      ...
      foreach ($ocsComputer as $app) {
         ...
         $customapp                 = Glpi\Toolbox\Sanitizer::sanitize($app);
         $input                 = [];
         $input["computers_id"] = $computers_id;
         if (!empty($customapp) && isset($customapp["PATH"])) {
            $input["path"]        = $customapp["PATH"];
            $input["text"]        = (isset($customapp["TEXT"]) ? $customapp["TEXT"] : '');
    ...
    */
   static function resetCustomapp($glpi_computers_id, $uninstall_history) {
      ...
      if ($plugin_ocsinventoryng_ocsservers_id > 0
         ...
         if (($item->getType() == 'Computer')
            ...
            if ($_SESSION['glpishow_count_on_tabs']) {
               $dbu = new DbUtils();
               $nb = $dbu->countElementsInTable('glpi_plugin_ocsinventoryng_customapps',
...
*/
   ...
   static function showForComputer(Computer $comp, $withtemplate = '') {
      ...
      if ($result = $DB->request('glpi_plugin_ocsinventoryng_customapps', ['computers_id' => $ID, 'ORDER' => 'path'])) {
         echo "<table class='tab_cadre_fixehov'>";
         $colspan = 5;
