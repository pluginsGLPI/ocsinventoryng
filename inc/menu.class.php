    ...
    static function getMenuContent()
    {
       ...
       // Import
        if (Session::haveRight("plugin_ocsinventoryng_import", READ)) {
            $menu['links']["<i class='fas fa-plus fa-1x' title='" . __s('Import new computers', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/ocsng.import.php';
        }
       // Sync
        if (Session::haveRight("plugin_ocsinventoryng_sync", READ)) {
            $menu['links']["<i class='fas fa-sync-alt fa-1x' title='" . __s('Synchronize computers already imported', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/ocsng.sync.php';
        }
       // Thread
        if ($usemassimport && Session::haveRight("plugin_ocsinventoryng", UPDATE)) {
            $menu['links']["<i class='fas fa-play fa-1x' title='" . __('Scripts execution of automatic actions', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/thread.php';
        }
       // Detail
        ...
        if ($log && Session::haveRight("plugin_ocsinventoryng_import", READ) && $usemassimport) {
            $menu['links']["<i class='fas fa-check fa-1x' title='" . __('Computers imported by automatic actions', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/detail.php';
        }
       // Notimported
        if (Session::haveRight("plugin_ocsinventoryng", UPDATE) && $usemassimport) {
            $menu['links']["<i class='fas fa-times fa-1x' title='" . __s('Computers not imported by automatic actions', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/notimportedcomputer.php';
           // networkport
            $menu['links']["<i class='fas fa-network-wired fa-1x' title='" . _n('Unknown imported network port type', 'Unknown imported network ports types', 2, 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/networkport.php';
        }
        if (Session::haveRight("plugin_ocsinventoryng_clean", UPDATE)
          || Session::haveRight("plugin_ocsinventoryng", UPDATE)) {
            if (Session::haveRight("plugin_ocsinventoryng", UPDATE)) {
               // Deleted_equiv
                $menu['links']["<i class='fas fa-trash fa-1x' title='" . __s('Clean OCSNG deleted computers', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/deleted_equiv.php';
            }
           // Clean
            $menu['links']["<i class='fas fa-broom fa-1x' title='" . __s('Clean links between GLPI and OCSNG', 'ocsinventoryng') . "'></i>"] = PLUGIN_OCS_WEBDIR . '/front/ocsng.clean.php';
        }
