...
class PluginOcsinventoryngNetworkPort extends NetworkPortInstantiation
{
    ...
    public static function importNetwork($cfg_ocs, $ocsComputer, $computers_id, $entities_id)
    {
        ...
        foreach ($ocsNetworks as $line) {
            ...
            $name = PluginOcsinventoryngOcsProcess::encodeOcsDataInUtf8(
                $cfg_ocs["ocs_db_utf8"],
                $line['DESCRIPTION']
            );
    ...
    public static function displayInvalidList()
    {
        ...
        if (count($iterator) > 0) {
            ...
            foreach ($iterator as $type) {
                ...
                $criteria = [
                    'SELECT' => ['TYPEMIB', 'TYPE',   new QueryExpression(
                        "GROUP_CONCAT(DISTINCT `speed` SEPARATOR '#' AS ' . $DB->quoteName('speed')'."
                    )],
                    'FROM' => 'glpi_plugin_ocsinventoryng_networkports',
                    'WHERE' => [
                        'TYPE' => $type['TYPE'],
                ...
                foreach ($typemib_results as $typemib) {
                    ...
                    echo "<td>" . PluginOcsinventoryngNetworkPortType::getLinkToCreateFromTypeAndTypeMIB($typemib) . "</td>";
