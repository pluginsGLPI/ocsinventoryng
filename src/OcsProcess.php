...
use GlpiPlugin\Ocsinventoryng\Components\Device;
use GlpiPlugin\Ocsinventoryng\Components\Disk;
use GlpiPlugin\Ocsinventoryng\Components\Hardware;
use GlpiPlugin\Ocsinventoryng\Components\Monitor;
...
use GlpiPlugin\Ocsinventoryng\Components\Peripheral;
use GlpiPlugin\Ocsinventoryng\Components\Printer;
...
use GlpiPlugin\Ocsinventoryng\Components\Software;
use GlpiPlugin\Ocsinventoryng\Components\Teamviewer;
use GlpiPlugin\Ocsinventoryng\Components\Uptime;
use GlpiPlugin\Ocsinventoryng\Components\Virtualmachine;
 ...
 * Class OcsProcess
 */
class OcsProcess extends CommonDBTM
{
    ...
    public const HARDWARE_FL        = 0;//1
    public const BIOS_FL            = 1;//2
    public const MEMORIES_FL        = 2;//4
    public const SLOTS_FL           = 3;//8
    public const REGISTRY_FL        = 4;//16
    public const CONTROLLERS_FL     = 5;//32
    public const MONITORS_FL        = 6;//64
    public const PORTS_FL           = 7;//128
    public const STORAGES_FL        = 8;//256
    public const DRIVES_FL          = 9;//512
    public const INPUTS_FL          = 10;//1024
    public const MODEMS_FL          = 11;//2048
    public const NETWORKS_FL        = 12;//4096
    public const PRINTERS_FL        = 13;//8192
    public const SOUNDS_FL          = 14;//16384
    public const VIDEOS_FL          = 15;//32768
    public const SOFTWARES_FL       = 16;//65536
    public const VIRTUALMACHINES_FL = 17; //131072
    public const CPUS_FL            = 18; //added into OCS2_1_VERSION_LIMIT - 262144

    public const SIMS_FL      = 19; //not used added into OCS2_1_VERSION_LIMIT - 524288
    public const BATTERIES_FL = 20; //not used  added into OCS2_2_VERSION_LIMIT - 1048576

    public const MAX_CHECKSUM = 524287;//262143;//With < 19 (with 20 : 2097151)

    public const FIELD_SEPARATOR = '$$$$$';
    public const IMPORT_TAG_078  = '_version_078_';

    // Class constants - Update result
    public const COMPUTER_IMPORTED      = 0; //Computer is imported in GLPI
    public const COMPUTER_SYNCHRONIZED  = 1; //Computer is synchronized
    public const COMPUTER_LINKED        = 2; //Computer is linked to another computer already in GLPI
    public const COMPUTER_FAILED_IMPORT = 3; //Computer cannot be imported because it matches none of the rules
    public const COMPUTER_NOTUPDATED    = 4; //Computer should not be updated, nothing to do
    public const COMPUTER_NOT_UNIQUE    = 5; //Computer import is refused because it's not unique
    public const COMPUTER_LINK_REFUSED  = 6; //Computer cannot be imported because a rule denies its import
    public const LINK_RESULT_IMPORT     = 0;
    public const LINK_RESULT_NO_IMPORT  = 1;
    public const LINK_RESULT_LINK       = 2;

    // Class constants - Update result
    public const SNMP_IMPORTED            = 10; //SNMP Object is imported in GLPI
    public const SNMP_SYNCHRONIZED        = 11; //SNMP is synchronized
    public const SNMP_LINKED              = 12; //SNMP is linked to another object already in GLPI
    public const SNMP_FAILED_IMPORT       = 13; //SNMP cannot be imported - no itemtype
    public const SNMP_NOTUPDATED          = 14; //SNMP should not be updated, nothing to do
    public const IPDISCOVER_IMPORTED      = 15; //IPDISCOVER Object is imported in GLPI
    public const IPDISCOVER_NOTUPDATED    = 16; //IPDISCOVER should not be updated, nothing to do
    public const IPDISCOVER_FAILED_IMPORT = 17; //IPDISCOVER cannot be imported - no itemtype
    public const IPDISCOVER_SYNCHRONIZED  = 18; //IPDISCOVER is synchronized
     ...
     */
    public static function manageDeleted($plugin_ocsinventoryng_ocsservers_id, $redirect = true)
    {
        global $DB;

        if (!(OcsServer::checkOCSconnection($plugin_ocsinventoryng_ocsservers_id)
              && OcsServer::checkVersion($plugin_ocsinventoryng_ocsservers_id))) {
            return false;
        }
        ...
        if (count($deleted)) {
            foreach ($deleted as $del => $equiv) {
                if (!empty($equiv) && $equiv != "NULL") { // New name ($equiv = VARCHAR)
                    ...
                    if (strpos($equiv, "-") !== false) {
                        ...
                        if (isset($res['COMPUTERS']) && count($res['COMPUTERS'])) {
                            if (isset($res['COMPUTERS']['META']) && is_array(isset($res['COMPUTERS']['META']))) {
                                ...
                                $DB->doQuery($query);
                                $ocsClient->setChecksum($data['CHECKSUM'] | OcsClient::CHECKSUM_HARDWARE, $data['ID']);
                            }
                            // } else {
                    ...
                    if (isset($data)) {
                        ...
                        if ($res_id = $DB->doQuery($sql_id)) {
                            if ($DB->numrows($res_id) > 0) {
                                ...
                                $changes[2] = $data["ID"];
                                Ocslink::history(
                                    $DB->result($res_id, 0, "computers_id"),
                                    $changes,
                                    Ocslink::HISTORY_OCS_IDCHANGED
                                );
                            }
                        }
                    }
                    ...
                    Ocslink::cleanLinksFromList($plugin_ocsinventoryng_ocsservers_id, $ocslinks_toclean);
                }
     ...
     * */
    public static function getOcsFieldsMatching()
    {
     ...
     * @return array
     */
    public static function getComputerInformations(
        $ocs_fields,
        $cfg_ocs,
        $entities_id,
        $groups_id = 0,
        ...
        foreach ($ocs_fields_matching as $glpi_field => $ocs_field) {
            ...
            if (isset($ocs_fields[$ocs_section]) && is_array($ocs_fields[$ocs_section])) {
                ...
                } elseif (isset($ocs_fields[$ocs_section][0])
            ...
            if (!is_null($ocs_val)) {
                ...
                if ($table != '') {
                    ...
                    foreach ($item->additional_fields_for_dictionnary as $field) {
                        ...
                        } elseif (isset($ocs_fields[$additional_ocs_section][0][$additional_ocs_field])) {
     ...
     * @throws \GlpitestSQLError
     */
    public static function processComputer($process_params)
    {
        global $DB;
     ...
     * @throws \GlpitestSQLError
     */
    public static function importComputer($import_params)
    {
        ...
        $ocsComputer = $ocsClient->getComputer($ocsid, [
            'DISPLAY' => [
                ...
                'WANTED'   => OcsClient::WANTED_ACCOUNTINFO,
            ],
        ]);
        ...
        if (isset($data['entities_id']) && $data['entities_id'] >= 0) {
            ...
            if (!is_null($ocsComputer)) {
                ...
                $is_recursive = ($data['is_recursive'] ?? 0);
                $input        = self::getComputerInformations(
                    $computer,
                    OcsServer::getConfig($plugin_ocsinventoryng_ocsservers_id),
                    $data['entities_id'],
                    $is_recursive
                );

                Hardware::getFields($ocsComputer, $cfg_ocs, $input);
                //import from computer import list
                if ($computers_id > 0) {
                ...
                $ocsClient->setChecksum(OcsClient::CHECKSUM_ALL, $ocsid);
                ...
                if ($computers_id) {
                    if ($cfg_ocs['dohistory'] == 1) {
                        ...
                        $changes[1] = "";
                        $changes[2] = $ocsid;
                        Ocslink::history($computers_id, $changes, Ocslink::HISTORY_OCS_IMPORT);
                    }
     ...
     * @throws \GlpitestSQLError
     */
    public static function linkComputer($link_params)
    {
        global $DB;
        ...
        $options     = [
            "DISPLAY" => [
                "CHECKSUM" => OcsClient::CHECKSUM_BIOS | OcsClient::CHECKSUM_NETWORK_ADAPTERS,
            ],
        ];
            ...
            if (!$ocsClient->getIfOCSComputersExists($data['ocsid'])) {
                ...
                if ($DB->doQuery($query)) {
                    if ($cfg_ocs['dohistory'] == 1) {
                        ...
                        //New ocsid
                        $changes[2] = $ocsid;
                        Ocslink::history($computers_id, $changes, Ocslink::HISTORY_OCS_IDCHANGED);
                    }
                }
            }
        }
        ...
        if ($ocs_id_change || !$ocs_link_exists) {
            ...
            if ($ocs_id_change
                || ($idlink = Ocslink::ocsLink($link_params))) {
                $comp = new Computer();
                ...
                if ($ocs_id_change && $cfg_ocs['dohistory'] == 1) {
                    ...
                    $changes[1] = "";
                    $changes[2] = $ocsid;
                    Ocslink::history($computers_id, $changes, Ocslink::HISTORY_OCS_LINK);
                }
                if ($idlink) {
                    $sync_params = ['ID'                                  => $idlink,
                        'plugin_ocsinventoryng_ocsservers_id' => $plugin_ocsinventoryng_ocsservers_id,
     ...
     */
    public static function transferComputer($line_links)
    {

        $ocsClient   = OcsServer::getDBocs($line_links["plugin_ocsinventoryng_ocsservers_id"]);
        $cfg_ocs     = OcsServer::getConfig($line_links["plugin_ocsinventoryng_ocsservers_id"]);
        $ocsComputer = $ocsClient->getComputer($line_links["ocsid"]);

        $values = [];
        Hardware::getFields($ocsComputer, $cfg_ocs, $values, $line_links['computers_id']);
        ...
        Hardware::updateComputerFields($line_links, $data, $cfg_ocs);
    }
     ...
     * @throws \GlpitestSQLError
     */
    public static function synchronizeComputer($sync_params, $transfer = true)
    {
        global $DB;
        ...
        if ($DB->numrows($result) == 1) {
            ...
            if (is_array($computer_ocs)
                ...
                if (Entity::getUsedConfig('transfers_strategy', $comp->fields['entities_id'], 'transfers_id', 0) > 0
                    ...
                    $values = [];
                    Hardware::getFields($computer_ocs, $cfg_ocs, $values, $line['computers_id']);
                    ...
                    Hardware::updateComputerFields($line, $data, $cfg_ocs);
                }
                ...
                if ($force || $computer_ocs["META"]["CHECKSUM"] > self::MAX_CHECKSUM) {
                    ...
                    OcsServer::getDBocs($plugin_ocsinventoryng_ocsservers_id)->setChecksum($ocs_checksum, $line['ocsid']);
                } else {
                    if (isset($computer_ocs['OFFICEPACK'])) {
                        $ocsClient->setChecksum($computer_ocs["META"]["CHECKSUM"] | OcsClient::CHECKSUM_SOFTWARE, $line['ocsid']);
                    }
                ...
                if ($mixed_checksum) {
                    ...
                    if ($mixed_checksum & pow(2, self::HARDWARE_FL)) {
                        ...
                        $ocsCheck[]          = OcsClient::CHECKSUM_HARDWARE;
                    }
                    if ($mixed_checksum & pow(2, self::BIOS_FL)) {
                        $updates['bios'] = true;
                        $ocsCheck[]      = OcsClient::CHECKSUM_BIOS;
                        if ($cfg_ocs["import_device_motherboard"]
                            && $cfg_ocs['ocs_version'] >= OcsServer::OCS2_2_VERSION_LIMIT) {
                            $updates['mb'] = true;
                        }
                    }
                    ...
                    if ($mixed_checksum & pow(2, self::CPUS_FL)) {
                        if ($cfg_ocs["import_device_processor"]
                            && !($cfg_ocs['ocs_version'] < OcsServer::OCS2_1_VERSION_LIMIT)) {
                            $updates['cpus'] = true;
                            $ocsCheck[]      = OcsClient::CHECKSUM_CPUS;
                        }
                    }
                    if ($mixed_checksum & pow(2, self::VIDEOS_FL)) {
                        if ($cfg_ocs["import_device_gfxcard"]) {
                            $updates['videos'] = true;
                            $ocsCheck[]        = OcsClient::CHECKSUM_VIDEO_ADAPTERS;
                        }
                    }
                    if ($mixed_checksum & pow(2, self::SOUNDS_FL)) {
                        if ($cfg_ocs["import_device_sound"]) {
                            $updates['sounds'] = true;
                            $ocsCheck[]        = OcsClient::CHECKSUM_SOUND_ADAPTERS;
                        }
                    }
                    if ($mixed_checksum & pow(2, self::NETWORKS_FL)) {
                        if ($cfg_ocs["import_device_iface"]
                            ...
                            $ocsCheck[]          = OcsClient::CHECKSUM_NETWORK_ADAPTERS;
                        }
                    }
                    if ($mixed_checksum & pow(2, self::MODEMS_FL)
                        ...
                        if ($cfg_ocs["import_device_modem"]) {
                            $updates['modems'] = true;
                            $ocsCheck[]        = OcsClient::CHECKSUM_MODEMS;
                        }
                    }
                    ...
                    if (!isset($ocsWanted)) {
                        $ocsWanted = OcsClient::WANTED_ACCOUNTINFO;
                    }
                    ...
                    if (isset($ocsComputer['HARDWARE'])) {
                        OcsAdminInfosLink::updateAdministrativeInfoUseDate(
                            $line['computers_id'],
                            $plugin_ocsinventoryng_ocsservers_id,
                            $computer_updates,
                    ...
                    if (isset($ocsComputer['META'])) {
                        Ocslink::updateTag($line, $ocsComputer['META']);
                    }
                    ...
                    if ($updates['bios'] && isset($ocsComputer['BIOS'])) {
                        Device::updateDevices("Item_DeviceFirmware", $ocsComputer['BIOS'], $params_devices);
                    }
                    if ($updates['memories'] && isset($ocsComputer['MEMORIES'])) {
                        Device::updateDevices("Item_DeviceMemory", $ocsComputer['MEMORIES'], $params_devices);
                    }

                    if ($updates['storages'] && isset($ocsComputer['STORAGES'])) {
                        if (isset($updates['storages']["hdd"]) && $updates['storages']["hdd"]) {
                            Device::updateDevices("Item_DeviceHardDrive", $ocsComputer['STORAGES'], $params_devices);
                        }
                        if (isset($updates['storages']["drive"]) && $updates['storages']["drive"]) {
                            Device::updateDevices("Item_DeviceDrive", $ocsComputer['STORAGES'], $params_devices);
                        }
                    }

                    if ($updates['cpus'] && isset($ocsComputer['CPUS'])) {
                        Device::updateDevices("Item_DeviceProcessor", $ocsComputer['CPUS'], $params_devices);
                    }
                    if ($updates['videos'] && isset($ocsComputer['VIDEOS'])) {
                        Device::updateDevices("Item_DeviceGraphicCard", $ocsComputer['VIDEOS'], $params_devices);
                    }

                    if ($updates['mb'] && isset($ocsComputer['BIOS'])) {
                        Device::updateDevices("Item_DeviceMotherboard", $ocsComputer['BIOS'], $params_devices);
                    }
                    if ($updates['controllers'] && isset($ocsComputer['CONTROLLERS'])) {
                        Device::updateDevices("Item_DeviceControl", $ocsComputer['CONTROLLERS'], $params_devices);
                    }
                    if ($updates['sounds'] && isset($ocsComputer['SOUNDS'])) {
                        Device::updateDevices("Item_DeviceSoundCard", $ocsComputer['SOUNDS'], $params_devices);
                    }
                    if ($updates['networks'] && isset($ocsComputer['NETWORKS'])) {
                        Device::updateDevices("Item_DeviceNetworkCard", $ocsComputer['NETWORKS'], $params_devices);
                    }
                    if ($updates['modems'] && isset($ocsComputer['MODEMS'])) {
                        Device::updateDevices("Item_DevicePci", $ocsComputer['MODEMS'], $params_devices);
                    }
                    if ($updates['slots'] && isset($ocsComputer['SLOTS'])) {
                        Device::updateDevices("Item_DevicePci", $ocsComputer['SLOTS'], $params_devices);
                    }
                    if ($updates['ports'] && isset($ocsComputer['PORTS'])) {
                        Device::updateDevices("Item_DevicePci", $ocsComputer['PORTS'], $params_devices);
                    }
                    if ($updates['monitors'] && isset($ocsComputer["MONITORS"])) {
                        $monitor_params = ['entities_id'                         => $comp->fields["entities_id"],
                            ...
                            'force'                               => $force];
                        Monitor::importMonitor($monitor_params);
                    }
                    if ($updates['printers'] && isset($ocsComputer["PRINTERS"])) {
                        $printer_params = ['entities_id'                         => $comp->fields["entities_id"],
                            ...
                            'force'                               => $force];
                        Printer::importPrinter($printer_params);
                    }
                    if ($updates['inputs'] && isset($ocsComputer["INPUTS"])) {
                        $periph_params = ['entities_id'                         => $comp->fields["entities_id"],
                            ...
                            'force'                               => $force];
                        Peripheral::importPeripheral($periph_params);
                    }
                    if ($updates['softwares'] && isset($ocsComputer["SOFTWARES"])) {
                        ...
                        Software::updateSoftware(
                            $cfg_ocs,
                            $line['computers_id'],
                            $ocsComputer["SOFTWARES"],
                            $comp->fields["entities_id"],
                            $updates['officepack'],
                            ($ocsComputer['OFFICEPACK'] ?? []),
                            $force
                        );
                    }
                    ...
                    if ($updates['virtualmachines'] && isset($ocsComputer["VIRTUALMACHINES"])) {
                        //import vm
                        Virtualmachine::updateVirtualMachine(
                            $line['computers_id'],
                            $ocsComputer["VIRTUALMACHINES"],
                            $plugin_ocsinventoryng_ocsservers_id,
                            $cfg_ocs,
                            $force
                        );
                    ...
                    if ($updates['registry'] && isset($ocsComputer["REGISTRY"])) {
                        ...
                        RegistryKey::updateRegistry(
                            $line['computers_id'],
                            $ocsComputer["REGISTRY"],
                            $cfg_ocs,
                            1
                        );
                    }
                    ...
                    if ($updates['antivirus'] && isset($ocsComputer["SECURITYCENTER"])) {
                        ...
                        Antivirus::updateAntivirus(
                            $line['computers_id'],
                            $ocsComputer["SECURITYCENTER"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['winupdatestate'] && isset($ocsComputer["WINUPDATESTATE"])) {
                        ...
                        Winupdate::updateWinupdatestate(
                            $line['computers_id'],
                            $ocsComputer["WINUPDATESTATE"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['osinstall'] && isset($ocsComputer["OSINSTALL"])) {
                        ...
                        Osinstall::updateOSInstall(
                            $line['computers_id'],
                            $ocsComputer["OSINSTALL"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['proxysetting'] && isset($ocsComputer["NAVIGATORPROXYSETTING"])) {
                        ...
                        Proxysetting::updateProxysetting(
                            $line['computers_id'],
                            $ocsComputer["NAVIGATORPROXYSETTING"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['networkshare'] && isset($ocsComputer["NETWORKSHARE"])) {
                        ...
                        Networkshare::updateNetworkshare(
                            $line['computers_id'],
                            $ocsComputer["NETWORKSHARE"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['runningprocess'] && isset($ocsComputer["RUNNINGPROCESS"])) {
                        ...
                        Runningprocess::updateRunningprocess(
                            $line['computers_id'],
                            $ocsComputer["RUNNINGPROCESS"],
                            $cfg_ocs,
                            1
                        );
                    }
                    ...
                    if ($updates['winuser'] && isset($ocsComputer["WINUSERS"])) {
                        ...
                        Winuser::updateWinuser(
                            $line['computers_id'],
                            $ocsComputer["WINUSERS"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['teamviewer'] && isset($ocsComputer["TEAMVIEWER"])) {
                        ...
                        Teamviewer::updateTeamviewer(
                            $line['computers_id'],
                            $ocsComputer["TEAMVIEWER"],
                            $cfg_ocs,
                            1
                        );
                    }
                    if ($updates['uptime'] && isset($ocsComputer["UPTIME"])) {
                        //import uptime
                        Uptime::updateUptime($ID, $ocsComputer["UPTIME"]);
                    }
                    if ($updates['customapp'] && isset($ocsComputer["CUSTOMAPP"])) {
                        //import teamviewer entries
                        Customapp::updateCustomapp(
                            $line['computers_id'],
                            $ocsComputer["CUSTOMAPP"],
                            $cfg_ocs,
                            1
                        );
                    }
                ...
                $locks = Ocslink::getLocksForComputer($line['computers_id']);
                Ocslink::mergeOcsArray($line['computers_id'], $locks);
                ...
                if ($newchecksum > 0) {
                    if (isset($computer_ocs['OFFICEPACK'])) {
                        $ocsClient->setChecksum($newchecksum | OcsClient::CHECKSUM_SOFTWARE, $line['ocsid']);
                    } else {
     ...
     */
    public static function getGeneralIpAddress($plugin_ocsinventoryng_ocsservers_id, $computers_id)
    {
        if (!OcsServer::checkOCSconnection($plugin_ocsinventoryng_ocsservers_id)) {
            return '';
        }
     ...
     */
    public static function setEntityLock($entity)
    {
     ...
     * @param $fp
     */
    public static function removeEntityLock($entity, $fp)
    {
        flock($fp, LOCK_UN);
        fclose($fp);
     ...
     * @return array
     */
    public static function getAvailableStatistics($snmp = false, $ipdiscover = false)
    {
     ...
     * @param bool  $ipdiscover
     */
    public static function manageImportStatistics(&$statistics = [], $action = false, $snmp = false, $ipdiscover = false)
    {
        if (empty($statistics)) {
     ...
     * @param bool  $ipdiscover
     */
    public static function showStatistics($statistics = [], $finished = false, $snmp = false, $ipdiscover = false)
    {
        echo "<div class='alert alert-info d-flex alert-dismissible' role='alert'>";
        ...
        switch ($ma->getAction()) {
            ...
            case "plugin_ocsinventoryng_lock_ocsng_field":
                ...
                if ($input['field'] == 'all' || isset($fields[$input['field']])) {
                    foreach ($ids as $id) {
                        if ($input['field'] == 'all') {
                            if (Ocslink::addToOcsArray($id, array_flip($fields), "computer_update")) {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_OK);
                            } else {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_KO);
                            }
                        } else {
                            if (Ocslink::addToOcsArray($id, [$input['field']], "computer_update")) {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_OK);
                            } else {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_KO);
                            }
            ...
            case "plugin_ocsinventoryng_unlock_ocsng_field":
                ...
                if ($input['field'] == 'all' || isset($fields[$input['field']])) {
                    foreach ($ids as $id) {
                        if ($input['field'] == 'all') {
                            if (Ocslink::replaceOcsArray($id, [])) {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_OK);
                            } else {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_KO);
                            }
                        } else {
                            if (Ocslink::deleteInOcsArray($id, $input['field'], true)) {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_OK);
                            } else {
                                $ma->itemDone($item->getType(), $id, MassiveAction::ACTION_KO);
                            }
