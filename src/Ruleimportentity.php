...
/**
 * Class RuleImportEntity
 */
class RuleImportEntity extends CommonDBTM
{
     ...
     * @return int
     */
    public static function cronCheckRuleImportEntity($task)
    {
        global $DB, $CFG_GLPI;
        ...
        if ($CronTask->getFromDBbyName(RuleImportEntity::class, "CheckRuleImportEntity")) {
            ...
            if ($plugin_ocsinventoryng_ocsservers_id > 0) {
                ...
                foreach ($computers as $entities_id => $items) {
                    $message = $plugin_ocsinventoryng_ocsservers_name . ": <br />" .
                               sprintf(
                                   __('Items that do not meet the allocation rules for the entity %s: %s', 'ocsinventoryng'),
     ...
     * @return array
     */
    public static function checkRuleImportEntity($plugin_ocsinventoryng_ocsservers_id)
    {
        $data = [];
     ...
     * @return array
     */
    public static function getComputerOcsLink($plugin_ocsinventoryng_ocsservers_id)
    {
        $ocslink = new Ocslink();

        $ocslinks = $ocslink->find(
