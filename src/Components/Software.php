...
class Software
{
    ...
    */
    static function updateSoftware($cfg_ocs, $computers_id, $ocsComputer, $entity, $officepack, $ocsOfficePack, $force = 0)
    {
        ...
        if ($officepack) {
            Officepack::resetOfficePack($computers_id, $uninstall_plugins_history);
        }
       ...
       // Read imported software in last sync
        ...
        $criteria = [
            'SELECT' => ['glpi_items_softwareversions.id AS id',
            ...
            'WHERE' => [
                ...
                'glpi_items_softwareversions.items_id' => $computers_id,
                'is_dynamic' => 1
            ],
        ];
        ...
        foreach ($iterator as $data) {
            $imported[$data['id']] = strtolower($data['sname'] . OcsProcess::FIELD_SEPARATOR . $data['vname']);
        }
...
//                         AND `is_dynamic` = 1";
            ...
            $criteria = [
                'SELECT' => ['glpi_items_softwarelicenses.id AS id',
                    'glpi_softwares.name AS sname',
                    'glpi_softwarelicenses.name AS lname',
                    'glpi_softwareversions.id AS vid'],
                'FROM' => 'glpi_items_softwarelicenses',
                ...
                'WHERE' => [
                    ...
                    'glpi_items_softwarelicenses.items_id' => $computers_id,
                    'is_dynamic' => 1
                ],
            ];
        ...
        foreach ($ocsComputer as $software) {
           ...
           //               $software[$field] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $software[$field]);
           //            }
           //         }
            if (isset($software['NAME'])) {
                $software['NAME'] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $software['NAME']);
            }
            if (isset($software['PUBLISHER'])) {
                $software['PUBLISHER'] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $software['PUBLISHER']);
            }
            if (isset($software['VERSION'])) {
                $software['VERSION'] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $software['VERSION']);
            }
            ...
            if (!isset($res_rule["_ignore_import"])
             || !$res_rule["_ignore_import"]) {
               ...
               //     if not, GLPI will detect an uninstall (oldname) + install (newname)
                ...
                $id = array_search(
                    strtolower(stripslashes($modified_name . OcsProcess::FIELD_SEPARATOR . $modified_version)),
                    $imported
                );


                $isNewSoft = $soft->addOrRestoreFromTrash(
                    $modified_name,
                    $manufacturer,
                    $target_entity,
                    '',
        ...
        foreach ($imported as $id => $unused) {
            $item_softwareversion->delete(
                ['id' => $id, '_no_history' => !$uninstall_history],
                true,
                $uninstall_history
            );
           // delete cause a getFromDB, so fields contains values
            ...
            if ($dbu->countElementsInTable(
             ...
             && $dbu->countElementsInTable(
                 'glpi_softwarelicenses',
                 ["softwareversions_id_buy" => $verid]
             ) == 0) {
                $vers = new SoftwareVersion();
                if ($vers->getFromDB($verid)
                && $dbu->countElementsInTable(
                    'glpi_softwarelicenses',
                    ["softwares_id" => $vers->fields['softwares_id']]
                ) == 0
                && $dbu->countElementsInTable(
                    'glpi_softwareversions',
                    ["softwares_id" => $vers->fields['softwares_id']]
                  ...
                  // 1 is the current to be removed
                    $soft->putInTrash(
                        $vers->fields['softwares_id'],
                        __('Software deleted by OCSNG synchronization', 'ocsinventoryng')
                    );
                }
                $vers->delete(
                    ["id" => $verid, '_no_history' => !$uninstall_history],
                    true,
                    $uninstall_history
                );
            }
