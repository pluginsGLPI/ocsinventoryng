...
class Disk
{
     ...
     */
    public static function updateDisk($computers_id, $ocsComputer, $ocsservers_id, $bitlockerstatus, $ocsBitlockerStatus, $cfg_ocs, $force)
    {
        global $DB;
        ...
        if (count($logical_drives) > 0) {
            foreach ($logical_drives as $logical_drive) {
                ...
                if ($logical_drive['TOTAL'] > 0) {
                    ...
                    } elseif (in_array($logical_drive['FILESYSTEM'], ['ext2', 'ext3', 'ext4', 'ffs',
                    ...
                    } elseif (in_array($logical_drive['FILESYSTEM'], ['FAT', 'FAT32', 'NTFS', 'ReFS'])) {
                    ...
                    if (isset($disk['name']) && !empty($disk["name"])) {
                        ...
                        if ($bitlockerstatus && isset($ocsBitlockerStatus)) {
                            //import bitlocker
                            Bitlockerstatus::updateBitlocker(
                                $computers_id,
                                $ocsBitlockerStatus,
                                $disk,
                                $cfg_ocs,
     ...
     * @return void .
     */
    public static function resetDisks($glpi_computers_id, $ocsBitlockerStatus, $uninstall_history)
    {
        ...
        if (!empty($ocsBitlockerStatus)) {
            Bitlockerstatus::resetBitlocker($glpi_computers_id, $uninstall_history);
        }
    }
}
