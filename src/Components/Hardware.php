    ...
    public static function updateLockforComputer(CommonDBTM $item)
    {
        ...
        if ($item->fields["is_dynamic"]
            ...
            if ($cfg_ocs["use_locks"]) {
                ...
                Ocslink::mergeOcsArray($item->fields["id"], $updates);
            }
        }
    }
    ...
    public static function updateComputerHardware($options = [])
    {
        ...
        if (isset($options['HARDWARE'])) {
            ...
            if (intval($options['cfg_ocs']["import_general_domain"]) > 0) {
                $opt["domains_id"] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $hardware["WORKGROUP"]);
            ...
            if (intval($options['cfg_ocs']["import_general_contact"]) > 0
                ...
                if (!empty($hardware["USERID"])) {
                    $updates["contact"] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $hardware["USERID"]);
                }
            }
            ...
            if (intval($options['cfg_ocs']["import_general_name"]) > 0
                ...
                $updates["name"] = OcsProcess::encodeOcsDataInUtf8($is_utf8, $hardware["NAME"]);
            }
            ...
            if (intval($options['cfg_ocs']["import_general_comment"]) > 0
                ...
                if (!empty($hardware["DESCRIPTION"])
                    ...
                    $updates["comment"] .= OcsProcess::encodeOcsDataInUtf8($is_utf8, $hardware["DESCRIPTION"])
                        . "\r\n";
                }
                $updates["comment"] .= sprintf(__('%1$s: %2$s'), __('Swap', 'ocsinventoryng'), OcsProcess::encodeOcsDataInUtf8($is_utf8, $hardware["SWAP"]));
            }

            if ($options['cfg_ocs']['ocs_version'] >= OcsServer::OCS1_3_VERSION_LIMIT
                ...
                if (isset($data['locations_id'])) {
                    ...
                    if ($location->getFromDB($data['locations_id'])) {
                        ...
                        if ($location->fields['entities_id'] == $computer->fields['entities_id']
                            ...
                            $ko = 0;
                            $locks = Ocslink::getLocksForComputer($line_links['computers_id']);
