 ...
 * Class Teamviewer
 */
class Teamviewer extends \CommonDBTM
{
    ...
    {
        return "glpi_plugin_ocsinventoryng_teamviewers";
    }
    /**
...
*/
    public static function showForComputer(Computer $comp, $withtemplate = '')
    {
        ...
        if ($result = $DB->request([
            ...
            if ($result->numrows() != 0) {
                ...
                WHERE `glpi_links_itemtypes`.`itemtype`='GlpiPlugin\\Ocsinventoryng\\Components\\Teamviewer' " .
    ...
    public static function showForSimpleForItem(Computer $comp, $withtemplate = '')
    {
        ...
        if ($iterator = $DB->request([
            'FROM' => 'glpi_plugin_ocsinventoryng_teamviewers',
            'WHERE' => ['computers_id' => $ID]])) {
            if (count($iterator)) {
                $data = $iterator->next();
