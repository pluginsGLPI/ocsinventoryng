...
JAVASCRIPT;
   ...
   function pre_updateInDB() {
      ...
      if ($this->fields["import_device_processor"] || $this->fields["import_general_contact"] || $this->fields["import_general_comment"] || $this->fields["import_general_domain"] || $this->fields["import_general_os"] || $this->fields["import_general_name"]) {
   ...
   static function updateComputer($ID, $plugin_ocsinventoryng_ocsservers_id, $dohistory, $force = 0) {
      ...
      if ($DB->numrows($result) == 1) {
         ...
         if ($computer) {
            ...
            if ($softwares) {
               // Get import software
               self::updateSoftware($line['computers_id'], $comp->fields["entities_id"], $ocsComputer, $plugin_ocsinventoryng_ocsservers_id, $cfg_ocs, (!$loghistory["history"] ? 0 : $dohistory));
            }
            if ($drives) {
               // Get import drives
            ...
            if ($registry) {
               ...
               self::updateRegistry($line['computers_id'], $ocsComputer, $plugin_ocsinventoryng_ocsservers_id, $cfg_ocs);
            }
            if ($virtualmachines) {
               // Get import vm
   ...
   static function updateBios($computers_id, $ocsComputer, $plugin_ocsinventoryng_ocsservers_id, $cfg_ocs, $computer_updates, $dohistory = 2, $entities_id = 0) {
      ...
      if (!is_null($computer)) {
         ...
         if (isset($computer["BIOS"])) {
            ...
            if ($cfg_ocs["import_general_serial"] && $cfg_ocs["import_general_serial"] > 0 && intval($cfg_ocs["import_device_bios"]) > 0 && !in_array("serial", $computer_updates)) {
               $compupdate["serial"] = self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $bios["SSN"]);
            }

            if (intval($cfg_ocs["import_general_model"]) > 0 && intval($cfg_ocs["import_device_bios"]) > 0 && !in_array("computermodels_id", $computer_updates)) {
            ...
            if (intval($cfg_ocs["import_general_manufacturer"]) > 0 && intval($cfg_ocs["import_device_bios"]) > 0 && !in_array("manufacturers_id", $computer_updates)) {
            ...
            if (intval($cfg_ocs["import_general_type"]) > 0 && intval($cfg_ocs["import_device_bios"]) > 0 && !empty($bios["TYPE"]) && !in_array("computertypes_id", $computer_updates)) {
   ...
   static function showComputersToClean($plugin_ocsinventoryng_ocsservers_id, $check, $start) {
      ...
      if (isset($computers['COMPUTERS']) && count($computers['COMPUTERS']) > 0) {
         ...
         case "Item_DeviceMotherboard":
            ...
            if ($ocsComputer) {
               ...
               if (isset($ocsComputer['BIOS'])) {
                  ...
                  $mb["manufacturers_id"] = Dropdown::importExternal('Manufacturer', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsComputer['BIOS']["MMANUFACTURER"]));
    ...
    * @return Nothing (void).
    * */
   static function updateSoftware($computers_id, $entity, $ocsComputer, $plugin_ocsinventoryng_ocsservers_id, array $cfg_ocs, $dohistory) {
      global $DB;
   ...
   static function importSnmp($ocsid, $plugin_ocsinventoryng_ocsservers_id, $lock = 0, $params) {
      ...
      if ($ocsSnmp['META']['ID'] == $ocsid && $p['itemtype'] != -1) {
         ...
         $loc_id = Dropdown::importExternal('Location', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['META']['LOCATION']));

         $dom_id = Dropdown::importExternal('Domain', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['META']['DOMAIN']));
         ...
         if ($id) {
            ...
            return array('status' => self::SNMP_IMPORTED,
               //'entities_id'  => $data['entities_id'],
            );
         } else {
            return array('status' => self::SNMP_FAILED_IMPORT,
               //'entities_id'  => $data['entities_id'],
            );
         }
      } else {
   ...
   static function addOrUpdateNetworkEquipment($plugin_ocsinventoryng_ocsservers_id, $itemtype, $ID = 0, $ocsSnmp, $loc_id, $dom_id, $action) {
      ...
      if (!empty($ocsSnmp['SWITCH'])) {
         ...
         if ($cfg_ocs['importsnmp_manufacturer']) {
            $man_id = Dropdown::importExternal('Manufacturer', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['SWITCH'][0]['MANUFACTURER']));
            $input['manufacturers_id'] = $man_id;
         }
         
         if ($cfg_ocs['importsnmp_firmware']) {
            $firm_id = Dropdown::importExternal('NetworkEquipmentFirmware', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['SWITCH'][0]['FIRMVERSION']));
      ...
      if ($id_network > 0 
         ...
         if (isset($ocsSnmp['POWERSUPPLIES']) 
            ...
            $man_id = Dropdown::importExternal('Manufacturer', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['POWERSUPPLIES'][0]['MANUFACTURER']));
         ...
         if (isset($ocsSnmp['FANS']) 
            ...
            $man_id                  = Dropdown::importExternal('Manufacturer', self::encodeOcsDataInUtf8($cfg_ocs['ocs_db_utf8'], $ocsSnmp['FANS'][0]['MANUFACTURER']));
   ...
   static function updateSnmp($ID, $plugin_ocsinventoryng_ocsservers_id) {
      ...
      if ($itemtype == "Printer") {
         ...
         return array('status' => self::SNMP_SYNCHRONIZED,
            //'entities_id'  => $data['entities_id'],
         );
      ...
      return array('status' => self::SNMP_NOTUPDATED,
         //'entities_id'  => $data['entities_id'],
      );
   }
   ...
   static function linkSnmpDevice($ocsid, $plugin_ocsinventoryng_ocsservers_id, $params) {
      ...
      //|| 
         ...
         return array('status' => self::SNMP_LINKED,
            //'entities_id'  => $data['entities_id'],
         );
      }
      /*
